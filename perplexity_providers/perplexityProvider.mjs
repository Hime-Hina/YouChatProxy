...
class PerplexityProvider {
    ...
    for (const originalUsername of Object.keys(this.sessions)) {
      ...
      try {
        ...
        if (process.env.USE_MANUAL_LOGIN === 'true') {
          ...
          await sleep(3000);
          console.log(
            `请在打开的浏览器窗口中手动登录 Perplexity.ai (session #${session.configIndex})`,
          );
  ...
  async checkAccountStatus(page) {
    if (
      process.env.USE_MANUAL_LOGIN !== 'true' &&
      process.env.INCOGNITO_MODE === 'true'
    ) {
      try {
        await page.waitForSelector(
          'div[class*="inline-flex w-full cursor-pointer select-none items-center justify-start gap-sm rounded-full"]',
          { timeout: 2000 },
        );

        // 检查无痕模式图标
        const isIncognito = await page.evaluate(() => {
          const incognitoElement = document.querySelector(
            'div[class*="inline-flex w-full cursor-pointer select-none items-center justify-start gap-sm rounded-full"]',
          );
          return (
            incognitoElement && incognitoElement.innerHTML.includes('viewBox="0 0 24 24"')
          );
        });
      ...
      try {
        await page.waitForSelector('div[class*="relative flex items-center gap-x-xs"]', {
          timeout: 2000,
        });

        const isPro = await page.evaluate(() => {
          const svgElement = document.querySelector(
            'div[class*="relative flex aspect-square"] svg',
          );
          return (
            svgElement && svgElement.innerHTML.includes('fill-super dark:fill-superDark')
          );
        });

        const username = await page.evaluate(() => {
          const usernameElement = document.querySelector(
            'div[class*="relative flex items-center gap-x-xs"] div',
          );
  ...
  async validateSession(page) {
    ...
    if (accountStatus.username || accountStatus.isIncognito) {
      if (accountStatus.isIncognito) {
        ...
        console.log(
          `账号 ${accountStatus.username} 登录有效${
            accountStatus.isPro ? '（Pro 账号）' : ''
          }`,
        );
      }
