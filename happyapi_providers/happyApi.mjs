...
class HappyApiProvider {
  ...
  async waitForManualLogin(page) {
    return new Promise((resolve) => {
      ...
      page.on('request', (request) => {
        if (
          request.url() === 'https://happyapi.org/api/v1/auths/signin' &&
          request.method() === 'POST'
        ) {
          console.log('检测到登录请求');
          checkLoginStatus();
  ...
  async checkLoginStatus(page) {
    try {
      const userNameElement = await page.waitForSelector('div.self-center.font-medium', {
        timeout: 5000,
      });
  ...
  async getCompletion({ username, messages, stream = false, proxyModel }) {
    ...
    //刷新页面
    await session.page.goto('https://happyapi.org', {
