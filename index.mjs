...
app.post('/v1/chat/completions', OpenAIApiKeyAuth, (req, res) => {
  ...
  req.on('end', async () => {
    ...
    console.log(`使用会话：${selectedSession}`);
    ...
    console.log(`使用模型：${jsonBody.model}`);
      ...
      completion.on('completion', (id, text) => {
        if (jsonBody.stream) {
          ...
          res.write(
            JSON.stringify({
              id: id,
              content: [{ text: text }, { id: 'string', name: 'string', input: {} }],
              model: proxyModel,
...
const createTunnel = async (tunnelType, port) => {
  ...
  if (tunnelType === 'localtunnel') {
    ...
    return createNgrok(
      ...
      process.env.NGROK_CUSTOM_DOMAIN,
